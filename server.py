...
class AttPublicRepo(Resource):
    def get(self):
        ...
        for index, i in df_repos.iterrows():
            r1 = "https://api.github.com/repos/att/"+i[0]+"/issues?state=open" #Hitting the github url for att repo to fetch issues data 
        ...
        for index, i in df_issues.iterrows():
            req2 = requests.get(i[2],auth=(username, password)) #Hitting the comments url to fetch comments information
